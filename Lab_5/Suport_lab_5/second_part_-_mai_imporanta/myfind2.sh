...
function parcurgere_director ()
{
    ...
    do
        ...
        if [ -d $cale ] ; then
            ...
            # wc -l => returneaza numarul de linii din fisierul $cale

            let nr_linii+=rezl
            rezc=`wc -c $cale | cut -d" " -f1`   #sau, echivalent: rezc=$(cat $cale | wc -c)
            # wc -c => returneaza numarul de octeti din fisierul $cale
            #se face acel cut pentru ca comanda cut returneaza forma urmatoare '705 call2find1.sh', si noua ne trebuie doar numarul de octeti, deci o sa taiem prima coloana sa ramanem cu numarul de octeti
